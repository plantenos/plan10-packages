# The PKGINST source packages manual

This article contains an exhaustive manual of how to create new source
packages for PKGINST, the `Plan10 Linux` native packaging system.
...
* [Introduction](#Introduction)
	...
	* [PKGINST Triggers](#pkginst_triggers)
...
The `plan10-packages` repository contains all `source` packages that are the
recipes to download, compile and build binary packages for `Plan10`.
...
`variables` and `functions` that are processed by `pkginst-src` (the package builder)
...
in a directory matching `$pkgname`, Example: `plan10-packages/srcpkgs/foo/template`.

If everything went fine after running

    $ ./pkginst-src pkg <pkgname>

a binary package named `foo-1.0_1.<arch>.pkginst` will be generated in the local repository
`hostdir/binpkgs`.
...
permitted in the Plan10 Linux repository. Exceptions to the list are possible,
...
`pkginst-src` supports running just the specified phase, and if it ran
successfully, the phase will be skipped later (unless its work directory
`${wrksrc}` is removed with `pkginst-src clean`).
...
in contrast to many other distributions, plan10 doesn't lowercase package names.
...
The following functions are defined by `pkginst-src` and can be used on any template:
...
The following variables are defined by `pkginst-src` and can be used on any template:

- `makejobs` Set to `-jX` if `PKGINST_MAKEJOBS` is defined, to allow parallel jobs with `GNU make`.
...
- `CROSS_BUILD` Set if `pkginst-src` is cross compiling a package.
...
- `PKGINST_BUILDDIR` Directory to store the `source code` of the source package being processed,
set to `<masterdir>/builddir`. The package `wrksrc` is always stored
in this directory such as `${PKGINST_BUILDDIR}/${wrksrc}`.

- `PKGINST_MACHINE` The machine architecture as returned by `pkginst-uhelper arch`.

- `PKGINST_ENDIAN` The machine's endianness ("le" or "be").

- `PKGINST_LIBC` The machine's C library ("glibc" or "musl").

- `PKGINST_WORDSIZE` The machine's word size in bits (32 or 64).

- `PKGINST_SRCDISTDIR` Full path to where the `source distfiles` are stored, i.e `$PKGINST_HOSTDIR/sources`.

- `PKGINST_SRCPKGDIR` Full path to the `srcpkgs` directory.

- `PKGINST_TARGET_MACHINE` The target machine architecture when cross compiling a package.

- `PKGINST_TARGET_ENDIAN` The target machine's endianness ("le" or "be").

- `PKGINST_TARGET_LIBC` The target machine's C library ("glibc" or "musl").

- `PKGINST_TARGET_WORDSIZE` The target machine's word size in bits (32 or 64).

- `PKGINST_FETCH_CMD` The utility to fetch files from `ftp`, `http` of `https` servers.

- `PKGINST_WRAPPERDIR` Full path to where pkginst-src's wrappers for utilities are stored.

- `PKGINST_CROSS_BASE` Full path to where cross-compile dependencies are installed, varies according to the target architecture triplet. i.e `aarch64` -> `aarch64-unknown-linux-gnu`.

- `PKGINST_RUST_TARGET` The target architecture triplet used by `rustc` and `cargo`.
...
and `PKGINST_MAKEJOBS` has no effect.
...
- `nodebug` If enabled -dbg packages won't be generated even if `PKGINST_DEBUG_PKGS` is set.
...
- `restricted` If set, pkginst-src will refuse to build the package unless
`etc/conf` has `PKGINST_ALLOW_RESTRICTED=yes`. The primary builders for Plan10
...
- `reverts` pkginst supports a unique feature which allows to downgrade from broken
...
It is used in the `x11-fonts` pkginst-trigger to rebuild the font cache during install/removal
of the package.
...
that will be installed and removed by the `dkms` pkginst-trigger with the install/removal of the
package.
...
package metadata and can be queried with `pkginst-query` by users.
...
between them, understand this: Plan10 Linux cross compiles for many arches.
...
the difference is that they are only installed when `PKGINST_CHECK_PKGS` is defined.

The final variable, `depends`, is for those things the package needs at
runtime and without which is unusable, and that pkginst can't auto-detect.
...
`./pkginst-src update-check <pkgname>`. In some cases you need to override
...
To handle this, pkginst-src has patching functionality. It will look for all files
...
pkginst-src will only apply patches named in the `series` file.
...
`plan10-packages/common/build-style` directory. Please note that required packages
...
- For do_fetch, post_fetch: `PKGINST_BUILDDIR`.
...
additional features; The PKGINST source packages collection allows you to do this with some simple tweaks
...
The supported build options for a source package can be shown with `pkginst-src`:

    $ ./pkginst-src show-options foo

Build options can be enabled with the `-o` flag of `pkginst-src`:

    $ ./pkginst-src -o option,option1 <cmd> foo
...
at the same time with `pkginst-src`:
    ...
    $ ./pkginst-src -o option,~option1,~option2 <cmd> foo

The build options can also be shown for binary packages via `pkginst-query(8)`:
...
Example: `PKGINST_PKG_OPTIONS_xorg_server=opt`.
...
Dependencies for ELF objects are detected automatically by `pkginst-src`, hence runtime
...
required version. The `shlibs` file in the `plan10-packages/common` directory
...
only checked if they exist as binary packages, and are built automatically by `pkginst-src` if
...
when the package is being built with `pkginst-src`.
...
that are always set by `pkginst` when the scripts are executed:
...
- `$CONF_FILE`: full path to `pkginst.conf`.
...
Plan10 Linux uses [runit](http://smarden.org/runit/) for booting and supervision of services.
...
Plan10 Linux on how to write services.
...
those detected automatically by `pkginst-src` and **depends**. Please note that
...
- `PKGINST_MAKEJOBS`: Value passed to the `-p` flag of `go install`, to
...
Plan10 Linux's repositories goes smoothly.
...
```
$pkgname is no longer provided by Plan10 Linux, and will be fully removed from the repos on $(date -d '+3 months' '+%F')
```

- After the specified time remove the package from the repository index
or contact a team member that can do so.

<a id="pkginst_triggers"></a>
### PKGINST Triggers

PKGINST triggers are a collection of snippets of code, provided by the `pkginst-triggers`
...
the installed files (`./pkginst-src show-files pkg`) before pushing new updates.

- Make sure that binaries are not stripped by the software, let pkginst-src do this;
otherwise the `debug` packages won't have debugging symbols.

<a id="contributing"></a>
### Contributing via git

Fork the plan10 `plan10-packages` git repository on github and clone it:

    $ git clone git@github.com:<user>/plan10-packages.git
...
to pull in new changes:
    ...
    $ git remote add upstream git://github.com/plantenos/plan10-packages.git
...
us at `#pkginst` via IRC at `irc.freenode.net`.
