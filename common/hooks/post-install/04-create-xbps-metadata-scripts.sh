# This hook generates PKGINST pkg metadata INSTALL/REMOVE scripts.
...
# \$3 = VERSION
# \$4 = UPDATE	[yes/no]
# \$5 = CONF_FILE (path to pkginst.conf)
# \$6 = ARCH (uname -m)
#
...
TRIGGERSDIR="./usr/libexec/pkginst-triggers"
ACTION="\$1"
PKGNAME="\$2"
VERSION="\$3"
UPDATE="\$4"
...
_EOF
	...
	if [ -n "$triggers" ]; then
		...
		echo "pre)" >> $tmpf
		for f in ${triggers}; do
			if [ ! -f $PKGINST_TRIGGERSDIR/$f ]; then
				rm -f $tmpf
		...
		for f in ${triggers}; do
			targets=$($PKGINST_TRIGGERSDIR/$f targets)
			for j in ${targets}; do
...
hook() {
	...
	if [ -n "${sourcepkg}" -a "${sourcepkg}" != "${pkgname}" ]; then
		# subpkg
		meta_install=${PKGINST_SRCPKGDIR}/${pkgname}/${pkgname}.INSTALL
		msg_install=${PKGINST_SRCPKGDIR}/${pkgname}/${pkgname}.INSTALL.msg
		meta_remove=${PKGINST_SRCPKGDIR}/${pkgname}/${pkgname}.REMOVE
		msg_remove=${PKGINST_SRCPKGDIR}/${pkgname}/${pkgname}.REMOVE.msg
	else
		# sourcepkg
		meta_install=${PKGINST_SRCPKGDIR}/${pkgname}/INSTALL
		msg_install=${PKGINST_SRCPKGDIR}/${pkgname}/INSTALL.msg
		meta_remove=${PKGINST_SRCPKGDIR}/${pkgname}/REMOVE
		msg_remove=${PKGINST_SRCPKGDIR}/${pkgname}/REMOVE.msg
	fi
