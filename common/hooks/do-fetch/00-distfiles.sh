...
contents_cksum() {
	...
	*) msg_error "$pkgver: unknown distfile suffix for $curfile.\n";;
	...
	tar|txz|tbz|tlz|tgz|crate)
		cksum=$($PKGINST_DIGEST_CMD <(tar xf "$curfile" --to-stdout))
		if [ $? -ne 0 ]; then
			msg_error "$pkgver: extracting $curfile to pipe.\n"
		fi
		;;
	gz)
		cksum=$($PKGINST_DIGEST_CMD <(gunzip -c "$curfile"))
		;;
	bz2)
		cksum=$($PKGINST_DIGEST_CMD <(bunzip2 -c "$curfile"))
		;;
	zip)
		if command -v unzip &>/dev/null; then
			cksum=$($PKGINST_DIGEST_CMD <(unzip -p "$curfile"))
			if [ $? -ne 0 ]; then
				msg_error "$pkgver: extracting $curfile to pipe.\n"
			fi
		else
	...
	rpm)
		if command -v rpmextract &>/dev/null; then
			cksum=$($PKGINST_DIGEST_CMD <(rpm2cpio "$curfile" | bsdtar xf - --to-stdout))
			if [ $? -ne 0 ]; then
				msg_error "$pkgver: extracting $curfile to pipe.\n"
			fi
		else
		...
		;;
	txt)
		cksum=$($PKGINST_DIGEST_CMD "$curfile")
		;;
	7z)
		if command -v 7z &>/dev/null; then
			cksum=$($PKGINST_DIGEST_CMD <(7z x -o "$curfile"))
			if [ $? -ne 0 ]; then
				msg_error "$pkgver: extracting $curfile to pipe.\n"
			fi
		else
		...
		;;
	gem)
		cksum=$($PKGINST_DIGEST_CMD <(tar -xf "$curfile" data.tar.gz --to-stdout | tar -xzO ))
		;;
	*)
...
verify_cksum() {
	...
	if [ "${cksum:0:1}" = "@" ]; then
		...
		filesum=$(${PKGINST_DIGEST_CMD} "$distfile")
		if [ "$cksum" != "$filesum" ]; then
			echo
			msg_red "SHA256 mismatch for '$curfile:'\n$filesum\n"
			errors=$((errors + 1))
		else
			if [ ! -f "$PKGINST_SRCDISTDIR/by_sha256/${cksum}_${curfile}" ]; then
				mkdir -p "$PKGINST_SRCDISTDIR/by_sha256"
				ln -f "$distfile" "$PKGINST_SRCDISTDIR/by_sha256/${cksum}_${curfile}"
			fi
			msg_normal_append "OK.\n"
		fi
	fi
...
link_cksum() {
	...
	if [ -n "$cksum" -a -f "$PKGINST_SRCDISTDIR/by_sha256/${cksum}_${curfile}" ]; then
		ln -f "$PKGINST_SRCDISTDIR/by_sha256/${cksum}_${curfile}" "$distfile"
...
try_mirrors() {
	...
	[ -z "$PKGINST_DISTFILES_MIRROR" ] && return
	basefile="${f##*/}"
	cksum=$(get_cksum $curfile $dfcount)
	for mirror in $PKGINST_DISTFILES_MIRROR; do
		...
		if [[ "$mirror" == *plan10* ]]; then
			# For distfiles.plan10.* append the subdirectory
	...
	for f in ${distfiles}; do
		...
		if [ -f "$distfile" ]; then
			...
			if [ "$cksum" = "$filesum" ]; then
				...
				find ${PKGINST_SRCDISTDIR} -inum ${inode} -delete -print
			fi
		fi
		dfcount=$((dfcount + 1))
	done
