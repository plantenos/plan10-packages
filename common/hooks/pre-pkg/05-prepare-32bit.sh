...
hook() {
	local destdir32=${PKGINST_DESTDIR}/${pkgname}-32bit-${version}
	...
	if [ -s "$PKGDESTDIR/rdeps" ]; then
		...
		for f in ${_deps}; do
			...
			pkgn="$($PKGINST_UHELPER_CMD getpkgdepname $f)"
			if [ -z "${pkgn}" ]; then
				pkgn="$($PKGINST_UHELPER_CMD getpkgname $f)"
				if [ -z "${pkgn}" ]; then
					msg_error "$pkgver: invalid dependency $f\n"
				fi
				pkgv="-$($PKGINST_UHELPER_CMD getpkgversion ${f})"
			else
				pkgv="$($PKGINST_UHELPER_CMD getpkgdepversion ${f})"
			fi
			...
			_arch=$($PKGINST_QUERY_CMD -R --property=architecture "$f")
			...
			if [ -z "$found" ]; then
				...
				# via binpkgs.
				_shprovides="$($PKGINST_QUERY_CMD -R --property=shlib-provides "$pkgn")"
				...
				if [ -s ${PKGINST_DESTDIR}/${pkgn}-${version}/shlib-provides ]; then
