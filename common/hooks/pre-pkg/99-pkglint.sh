# This hook checks for common issues related to plan10.

hook() {
	local error=0 filename= rev= libname= conflictPkg= conflictFile=
	local conflictRev= ignore= found= mapshlibs=$PKGINST_COMMONDIR/shlibs

	set +E

	# Check for forbidden directories that are symlinks in plan10.
	...
	for filename in $(<${PKGDESTDIR}/shlib-provides); do
		...
		grep -E "${_pattern}" $mapshlibs | { \
			while read conflictFile conflictPkg ignore; do
				...
				if [ -n "$ignore" -a "$ignore" != "$PKGINST_TARGET_MACHINE" ]; then
					continue
				...
				_revdeps=$($PKGINST_QUERY_XCMD -Rs ${libname}.so -p shlib-requires|cut -d ' ' -f1)
			...
			for f in ${provides}; do
				_vpkgname="$($PKGINST_UHELPER_CMD getpkgname ${f} 2>/dev/null)"
				_spkgname="$(grep "^${filename}" $mapshlibs | cut -d ' ' -f2)"
				_libpkgname="$($PKGINST_UHELPER_CMD getpkgname ${_spkgname} 2>/dev/null)"
