...
#	- Generates rdeps file with run-time dependencies for pkginst-create(1)
#	- Generates shlib-requires file for pkginst-create(1)

add_rundep() {
    local dep="$1" i= rpkgdep= _depname= found=

    _depname="$($PKGINST_UHELPER_CMD getpkgdepname ${dep} 2>/dev/null)"
    if [ -z "${_depname}" ]; then
        _depname="$($PKGINST_UHELPER_CMD getpkgname ${dep} 2>/dev/null)"
    fi

    for i in ${run_depends}; do
        rpkgdep="$($PKGINST_UHELPER_CMD getpkgdepname $i 2>/dev/null)"
        if [ -z "$rpkgdep" ]; then
            rpkgdep="$($PKGINST_UHELPER_CMD getpkgname $i 2>/dev/null)"
        fi
        if [ "${rpkgdep}" != "${_depname}" ]; then
            continue
        fi
        $PKGINST_UHELPER_CMD cmpver "$i" "$dep"
        rval=$?
        if [ $rval -eq 255 ]; then
...
store_pkgdestdir_rundeps() {
        if [ -n "$run_depends" ]; then
            ...
            for f in ${run_depends}; do
                ...
                if [ -z "$($PKGINST_UHELPER_CMD getpkgdepname ${_curdep} 2>/dev/null)" -a \
                     -z "$($PKGINST_UHELPER_CMD getpkgname ${_curdep} 2>/dev/null)" ]; then
                    _curdep="${_curdep}>=0"
                fi
...
hook() {
    ...
    # Disable trap on ERR, pkginst-uhelper cmd might return error... but not something
    # to be worried about because if there are broken shlibs this hook returns
    # error via msg_error().
    trap - ERR

    mapshlibs=$PKGINST_COMMONDIR/shlibs
    ...
    for f in ${verify_deps}; do
        ...
        if [ -z "$rdep" ]; then
            ...
            for j in ${rdep}; do
                _pkgname=$($PKGINST_UHELPER_CMD getpkgname "$j")
        ...
        _pkgname=$($PKGINST_UHELPER_CMD getpkgname "${_rdep}" 2>/dev/null)
        _rdepver=$($PKGINST_UHELPER_CMD getpkgversion "${_rdep}" 2>/dev/null)
