# This hook extracts $distfiles into $PKGINST_BUILDDIR if $distfiles and $checksum
# variables are set.
...
hook() {
	...
	for f in ${distfiles}; do
		...
		if [ ! -f $srcdir/$curfile ]; then
			msg_error "$pkgver: cannot find ${curfile}, use 'pkginst-src fetch' first.\n"
		fi
	done
	...
	for f in ${distfiles}; do
		...
		*) msg_error "$pkgver: unknown distfile suffix for $curfile.\n";;
		...
		txz|tbz|tlz|tgz|crate)
			...
			if [ $? -ne 0 ]; then
				msg_error "$pkgver: extracting $curfile into $PKGINST_BUILDDIR.\n"
			fi
			;;
		gz|bz2)
