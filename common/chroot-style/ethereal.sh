...
# 1. Symlink / to masterdir inside the plan10-packages repo
# 2. write the arch of the host system, as dictated by pkginst-uhelper arch
# into /.pkginst_chroot_init
#
...
# Docker example:
# $ mkdir -p /tmp/image
# $ pkginst-install -y -r /tmp/image \
#				 -R http://mirrors.servercentral.com/plan10/current \
#				 -S base-chroot
# $ tar -pC /tmp/image -c . | sudo docker import - plan10/masterdir
# $ rm -rf /tmp/image 
# # docker run --rm -it \
#			   -e PKGINST_CHROOT_CMD=ethereal \
#			   -e PKGINST_ALLOW_CHROOT_BREAKOUT=yes \
#			   -v $(pwd):/hostrepo plan10/masterdir \
#			   /bin/bash -c 'ln -s / /hostrepo/masterdir && /hostrepo/pkginst-src pkg <pkgname>'
#
...
if [ "${PKGINST_ALLOW_CHROOT_BREAKOUT}" != "yes" ]; then
	msg_red "chroot-style 'ethereal' requires PKGINST_ALLOW_CHROOT_BREAKOUT=yes\n"
...
fake_mount "$DISTDIR" "$MASTERDIR"/plan10-packages
...
# pkginst-src may send some other binds, parse them here
...
# Tell pkginst-src that we are "in the chroot"
...
rm -f "$MASTERDIR"/plan10-packages
mkdir -p "$MASTERDIR"/plan10-packages
