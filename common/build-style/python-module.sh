...
do_build() {
	...
	for pyver in $python_versions; do
		if [ -n "$CROSS_BUILD" ]; then
			...
			CFLAGS+=" -I${PKGINST_CROSS_BASE}/include/python${pyver}${pysufx} -I${PKGINST_CROSS_BASE}/usr/include"
			LDFLAGS+=" -L${PKGINST_CROSS_BASE}/lib/python${pyver} -L${PKGINST_CROSS_BASE}/usr/lib"
