...
do_build() {
	...
	if [ "$CROSS_BUILD" ]; then
		...
		_TARGET_PLATFORM="$(ruby -r \
			$(find ${PKGINST_CROSS_BASE}/usr/lib/ruby -iname rbconfig.rb) \
...
system("sed -i 's|^topdir.*|topdir = ${PKGINST_CROSS_BASE}/usr/include/ruby-\$(ruby_version)|' Makefile")
system("sed -i 's|^hdrdir.*|hdrdir = ${PKGINST_CROSS_BASE}/usr/include/ruby-\$(ruby_version)|' Makefile")
system("sed -i 's|^arch_hdrdir.*|arch_hdrdir = ${PKGINST_CROSS_BASE}/usr/include/ruby-\$(ruby_version)/\$(arch)|' Makefile")
...
EOF
	...
	for f in $distfiles; do
		if [ "${f##*.}" = "gem" ]; then
			$gem_cmd spec \
				"${PKGINST_SRCDISTDIR}/${pkgname}-${version}/${f##*/}" \
				--ruby > $gemspec
		fi
	done
