...
do_build() {
	...
	if [ "${go_mod_mode}" != "off" ] && [ -f go.mod ]; then
		...
		go install -p "$PKGINST_MAKEJOBS" -mod="${go_mod_mode}" -x -tags "${go_build_tags}" -ldflags "${go_ldflags}" ${go_package}
	else
		# Otherwise, build using GOPATH
		go get -p "$PKGINST_MAKEJOBS" -x -tags "${go_build_tags}" -ldflags "${go_ldflags}" ${go_package}
	fi
}
