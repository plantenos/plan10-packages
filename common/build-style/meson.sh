...
c = '${CC}'
cpp = '${CXX}'
ar = '${PKGINST_CROSS_TRIPLET}-gcc-ar'
nm = '${NM}'
ld = '${LD}'
strip = '${STRIP}'
readelf = '${READELF}'
...
rust = 'rustc'
g-ir-scanner = '${PKGINST_CROSS_BASE}/usr/bin/g-ir-scanner'
g-ir-compiler = '${PKGINST_CROSS_BASE}/usr/bin/g-ir-compiler'
g-ir-generate = '${PKGINST_CROSS_BASE}/usr/bin/g-ir-generate'
...
do_configure() {
	...
	if [ "$CROSS_BUILD" ]; then
		...
		# native: true, so we use PKGINST_CFLAGS, PKGINST_CXXFLAGS, PKGINST_CPPFLAGS and PKGINST_LDFLAGS
