...
# than $PKGINST_BUILDDIR/$wrksrc, one can use (relative to $wrksrc):
#
...
do_configure() {
	...
	mkdir -p $perlprefix
	if [ -d "$PKGINST_CROSS_BASE/usr/lib/perl5/core_perl" ]; then
		...
		sed -i -e "s;archlibexp => '\(.*\)';archlibexp => '${PKGINST_CROSS_BASE}\1';" \
	...
	if [ -f ${wrksrc}/Makefile.PL ]; then
		sed -i "s,/usr/include,${PKGINST_CROSS_BASE}/usr/include,g" ${wrksrc}/Makefile.PL
	fi
	...
	if [ -z "$perl_configure_dirs" ]; then
		...
		PERL_MM_USE_DEFAULT=1 GCC="$CC" CC="$CC" LD="$CC" \
			OPTIMIZE="$CFLAGS" \
			CFLAGS="$CFLAGS -I${PKGINST_CROSS_BASE}/usr/include" \
			LDFLAGS="$LDFLAGS -L${PKGINST_CROSS_BASE}/usr/lib -lperl" \
			LDDLFLAGS="-shared $CFLAGS -L${PKGINST_CROSS_BASE}/usr/lib" \
	...
	for i in ${perl_configure_dirs}; do
		...
		if [ -f $perlmkf ]; then
			...
			PERL_MM_USE_DEFAULT=1 GCC="$CC" CC="$CC" LD="$CC" \
				...
				CFLAGS="$CFLAGS -I${PKGINST_CROSS_BASE}/usr/include" \
				LDFLAGS="$LDFLAGS -L${PKGINST_CROSS_BASE}/usr/lib -lperl" \
				LDDLFLAGS="-shared $CFLAGS -L${PKGINST_CROSS_BASE}/usr/lib -lperl" \
