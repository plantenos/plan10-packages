...
purge_distfiles() {
	readonly HASHLEN=64
	if [ -z "$PKGINST_SRCDISTDIR" ]; then
		msg_error "The variable \$PKGINST_SRCDISTDIR is not set."
		exit 1
	fi
	#
	...
	#
	# Collect inodes of all distfiles in $PKGINST_SRCDISTDIR
	#
	declare -A inodes
	distfiles=($PKGINST_SRCDISTDIR/*/*)
	max=${#distfiles[@]}
	if [ -z "$max" ]; then
		msg_error "No distfiles files found in '$PKGINST_SRCDISTDIR'"
		exit 1
	fi
	cur=0
	percent=-1
	...
	hashes=($PKGINST_SRCDISTDIR/by_sha256/*)
