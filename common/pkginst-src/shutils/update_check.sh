...
update_check() {
    ...
    local update_override=$PKGINST_SRCPKGDIR/$PKGINST_TARGET_PKG/update
    ...
    if [ -r $update_override ]; then
        . $update_override
        if [ "$PKGINST_UPDATE_CHECK_VERBOSE" ]; then
            echo "using $PKGINST_TARGET_PKG/update overrides" 1>&2
        fi
    fi
    ...
    while IFS= read -r url; do
        ...
        if [ "$rx" ]; then
            ...
            if [ -n "$PKGINST_UPDATE_CHECK_VERBOSE" ]; then
                echo "(folder) fetching $urlpfx" 1>&2
            fi
            skipdirs=
            curl -A "pkginst-src-update-check/$PKGINST_SRC_VERSION" --max-time 10 -Lsk "$urlpfx" |
                grep -Po -i "$rx" |
    ...
    while IFS= read -r url; do
        ...
        curl -H 'Accept: text/html,application/xhtml+xml,application/xml,text/plain,application/rss+xml' -A "pkginst-src-update-check/$PKGINST_SRC_VERSION" --max-time 10 -Lsk "$url" |
            grep -Po -i "$rx"
        fetchedurls[$url]=yes
    done |
    tr _ . |
    ...
    while IFS= read -r found_version; do
        ...
        if $consider; then
            pkginst-uhelper cmpver "$original_pkgname-${version}_1" \
