...
setup_pkg_depends() {
    ...
    for j in ${depends}; do
        ...
        if [[ ${_rpkgname} == virtual ]]; then
            _pkgname=$(pkginst-uhelper getpkgname $_depname 2>/dev/null)
            [ -z "$_pkgname" ] && _pkgname="$_depname"
            if [ -s ${PKGINST_DISTDIR}/etc/virtual ]; then
                foo=$(egrep "^${_pkgname}[[:blank:]]" ${PKGINST_DISTDIR}/etc/virtual|cut -d ' ' -f2)
            elif [ -s ${PKGINST_DISTDIR}/etc/defaults.virtual ]; then
                foo=$(egrep "^${_pkgname}[[:blank:]]" ${PKGINST_DISTDIR}/etc/defaults.virtual|cut -d ' ' -f2)
            fi
            if [ -z "$foo" ]; then
            ...
            foo="$($PKGINST_UHELPER_CMD getpkgdepname ${_depname} 2>/dev/null)"
            if [ -z "$foo" ]; then
                foo="$($PKGINST_UHELPER_CMD getpkgname ${_depname} 2>/dev/null)"
                [ -z "$foo" ] && foo="${_depname}"
            fi
            _deps+="$foo "
        fi
...
# EEXIST  (17): file conflicts in transaction (PKGINST_FLAG_IGNORE_FILE_CONFLICTS unset)
...
install_pkg_from_repos() {
    ...
    mkdir -p $PKGINST_STATEDIR
    tmplogf=${PKGINST_STATEDIR}/pkginst_${PKGINST_TARGET_MACHINE}_bdep_${pkg}.log

    cmd=$PKGINST_INSTALL_CMD
    [[ $cross ]] && cmd=$PKGINST_INSTALL_XCMD
    $cmd ${PKGINST_SKIP_REMOTEREPOS:+-i} -AIy "$@" >$tmplogf 2>&1
    rval=$?
    ...
    case "$rval" in
        0) # success, check if there are errors.
           ...
           ;;
        *)
           [ -z "$PKGINST_KEEP_ALL" ] && remove_pkg_autodeps
...
check_installed_pkg() {
    ...
    pkgn="$($PKGINST_UHELPER_CMD getpkgname ${pkg})"
    [ -z "$pkgn" ] && return 2

    uhelper=$PKGINST_UHELPER_CMD
    [[ $cross ]] && uhelper=$PKGINST_UHELPER_XCMD
    iver="$($uhelper version $pkgn)"
    if [ $? -eq 0 -a -n "$iver" ]; then
        $PKGINST_CMPVER_CMD "${pkgn}-${iver}" "${pkg}"
...
install_pkg_deps() {
    ...
    [ -z "$pkgname" ] && return 2
    [ -z "$PKGINST_CHECK_PKGS" ] && unset checkdepends
    ...
    if [[ ${hostmakedepends} ]]; then
        ...
        for f in ${hostmakedepends}; do
            if [ ! -f $PKGINST_SRCPKGDIR/$f/template ]; then
        ...
        done < <($PKGINST_CHECKVERS_CMD ${PKGINST_SKIP_REMOTEREPOS:+-i} -D $PKGINST_DISTDIR -sm ${hostmakedepends})
    fi

    #
    # Host check dependencies.
    #
    if [[ ${checkdepends} ]] && [[ $PKGINST_CHECK_PKGS ]]; then
        # check validity
        ...
        done < <($PKGINST_CHECKVERS_CMD ${PKGINST_SKIP_REMOTEREPOS:+-i} -D $PKGINST_DISTDIR -sm ${checkdepends})
    fi
    ...
    if [[ ${makedepends} ]]; then
        ...
        done < <($PKGINST_CHECKVERS_XCMD ${PKGINST_SKIP_REMOTEREPOS:+-i} -D $PKGINST_DISTDIR -sm ${makedepends})
    fi
    ...
    if [[ ${depends} ]]; then
        ...
        done < <($PKGINST_CHECKVERS_XCMD ${PKGINST_SKIP_REMOTEREPOS:+-i} -D $PKGINST_DISTDIR -sm $_deps)
        unset _deps
    fi

    if [ -n "$PKGINST_BUILD_ONLY_ONE_PKG" ]; then
    ...
    for i in ${host_missing_deps[@]}; do
        ...
        (
        curpkgdepname=$($PKGINST_UHELPER_CMD getpkgname "$i" 2>/dev/null)
        setup_pkg $curpkgdepname
        exec env PKGINST_DEPENDENCY=1 PKGINST_BINPKG_EXISTS=1 \
            $PKGINST_LIBEXECDIR/build.sh $sourcepkg $pkg $target || exit $?
        ) || exit $?
        host_binpkg_deps+=("$i")
    done
    ...
    for i in ${missing_deps[@]}; do
        ...
        exec env PKGINST_DEPENDENCY=1 PKGINST_BINPKG_EXISTS=1 \
            $PKGINST_LIBEXECDIR/build.sh $sourcepkg $pkg $target $cross $cross_prepare || exit $?
        ) || exit $?
        binpkg_deps+=("$i")
    done
    ...
    for i in ${missing_rdeps[@]}; do
        # packages not found in repos, install from source.
        (
        curpkgdepname=$($PKGINST_UHELPER_CMD getpkgdepname "$i" 2>/dev/null)
